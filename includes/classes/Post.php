...
class Post {
    ...
    public function submitPost($body, $user_to, $imageName) {
        ...
        if ($check_empty != "") {
            ...
            //for posting & embedding YouTube video links onto page: remove spaces from text around links
            $body_array = preg_split("/\s+/", $body);

            foreach($body_array as $key => $value) {
                if (strpos($value, "www.youtube.com/watch?v=") !== false) {
                    //remove extra length from links (e.g. if user posts a video from a playlist)
                    $link = preg_split("!&!", $value);
                    $value = preg_replace("!watch\?v=!", "embed/", $link[0]); //link[0] to ensure we use shortest link possible
                    $value = "<br><iframe width=\'420\' height=\'315\' src=\'" . $value . "\'></iframe><br>";
                    $body_array[$key] = $value;
                }
            }

            //separate all links with space
            ...
            //replace any trending words that are not letter or number with empty space
